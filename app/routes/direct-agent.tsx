...
export default function DirectAgentPage() {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    if (!selectedCity.trim() || !connection) return;

    setResponse("");
    setError(null);
    setIsStreaming(true);

    // Send city to agent via WebSocket (prompt constructed server-side)
    connection.send(JSON.stringify({ city: selectedCity }));
  };
          ...
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="font-body text-stone-500 text-sm">
                Pattern 4: Bidirectional streaming via persistent connection
              </p>
            </div>
          </div>
          ...
          <CardContent>
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div className="flex items-center justify-between">
                ...
                <Button
                  type="submit"
                  disabled={!selectedCity.trim() || isStreaming}
          ...
          <CardContent className="font-body text-sm text-stone-600 space-y-3">
            ...
            <div className="flex items-start gap-3 p-3 rounded-xl bg-white border border-stone-300">
              ...
              <p>
                ...
                handler constructs the prompt server-side and streams chunks
                back.
              </p>
            </div>
          </CardContent>
